 ...
 * @author Steve
 */
public class CountdownClient extends javax.swing.JFrame {
    ...
    public CountdownClient(String nme, String ip, int prt) {
    ...
    void updateRound(int r) {
        ...
        if (r == 0) { //first round, betting
            ...
            jLabel13.setText("Current Round: Trade In Round (Up To 3 Cards)");  //only allows player to discard
            limit = 3;
            ...
            betButton.setEnabled(true);
            checkButton.setEnabled(true);
        } else if (r == 3) {    //fourth round, trade in
            jLabel13.setText("Current Round: Trade In Round (Up To 2 Cards)");  //only allows player to discard
            limit = 2;
            betButton.setEnabled(false);
            checkButton.setEnabled(false);
            discardButton.setEnabled(true);
        } else if (r == 4) {    //fifth round, betting
            jLabel13.setText("Current Round: Betting Round");//only allows player to bet
            discardButton.setEnabled(false);
            betButton.setEnabled(true);
            checkButton.setEnabled(true);
        } else if (r == 5) {    //sixth round, trade in
            jLabel13.setText("Current Round: Trade In Round (Up To 1 Card)");  //only allows player to discard
            limit = 1;
            betButton.setEnabled(false);
            checkButton.setEnabled(false);
            discardButton.setEnabled(true);
        } else if (r == 6) {    //seventh round, betting
            jLabel13.setText("Current Round: Betting Round");//only allows player to bet
            discardButton.setEnabled(false);
            betButton.setEnabled(true);
            checkButton.setEnabled(true);
                ...
                while (iterator.hasNext()) {
                    ...
                    if (nbytes == -1) {
                        ...
                        if ((line.indexOf("\n") != -1) || (line.indexOf("\r") != -1)) {
                            ...
                            } else if (c == 'r') {           //r is the round delimiter
                                round = line.charAt(1) - 48;
                                updateRound(round);
                            } else if (c == 'f') {           // f is the fold delimiter
...
// each counter keeps track of how many times user clicks on the card
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(CountdownClient.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            ...
            public void run() {
               // new CountdownClient().setVisible(true);
            }
        });
    }
